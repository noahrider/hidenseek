...
if "%VMWin%"=="" (
	if "%YYCEXE%"=="" (
		echo MultiClient: Cannot determine whether YYC or VM via existing files, falling back to old approach...
		if "%YYTARGET_runtime%"=="YYC" (
			set "_name=%YYprojectName%"
			set "_name=!_name: =_!"
			set "YYCEXE=!_name!.exe"
			echo MultiClient: YYC Path set to "!YYCEXE!"!
		) else (
			set "VMWin=%YYprojectName%.win"
			echo MultiClient: VM Path set to "!VMWin!"!
		)
	)
)
